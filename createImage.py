...
def packImages(rpath, result):
    ...
    for folder in dirs:
        ...
        if os.path.isdir(subf):
            ...
            for image in images:
                im = imageio.imread(os.path.join(subf, image))
...
def unpackImages(datafile, rpath, listFile=[]):
    ...
    for data,label in zip(data_x,data_y):
        ...
        if flist != []:
            ...
            imagename = str(i)+'.png'
        i+=1
        imageio.imsave(os.path.join(subf, imagename), data)
...
def makeAsymmetric(imageF, result):
    ...
    if nprandom.rand() < 0.5: # erase bar on right
        ...
        check_array = np.where(hist_x>1)
        if len(check_array[0]) != 0:
            tip_x_max = np.max(check_array)
            tip_x_min = np.min(check_array)
        else:
            tip_x_max = W//2
            tip_x_min = 0
        ...
        if tip_x_max <= 0:
            tip_x_max = 1
        if tip_x_max <= tip_x_min:
            tip_x_max = tip_x_min + 1
...
def shapeHalfImage(old, new, pattern):
    ...
    result = np.clip(result, 0, 255)  # Clip values to be within [0, 255]
    result = result.astype(np.uint8)  # Convert to uint8
    
    imageio.imsave(new, result)
...
def scaleHalfImage(old, new, pattern):
    ...
    if pattern == 2: # scale both half
        ...
        if np.sum(scaled>0) < 5:
            ...
            scaled = imageio.imread('temp.png')
    ...
    imageio.imsave(new, (scaled*255).astype(np.uint8))
...
def drawTriangle2(im, components, center, size, fill, focus, negative=False, partial=None, away=None, rotate=None, turn=False, otherfixed=False):
    ...
    dist = np.sqrt(np.dot(np.array(center)-np.array(focus), np.array(center)-np.array(focus)))
